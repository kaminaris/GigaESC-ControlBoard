...
void hw_init_gpio(void) {
	...
	/*
	palSetPadMode(
		HW_PROTECTION_CLEAR_GPIO,
		HW_PROTECTION_CLEAR_PIN,
	...
	*/
	// Disable	  
	palSetPadMode(
		HW_POWER_STAGE_DISABLE_GPIO,
		HW_POWER_STAGE_DISABLE_PIN,
		PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST
	);
	DISABLE_GATE();
	...
	// Hall filters
	palSetPadMode(HALL_FILTER_GPIO, HALL_FILTER_PIN, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
	HALL_FILTER_OFF();

	// Phase filters
	palSetPadMode(PHASE_FILTER_GPIO, PHASE_FILTER_PIN1, PAL_MODE_OUTPUT_OPENDRAIN);
	palSetPadMode(PHASE_FILTER_GPIO, PHASE_FILTER_PIN2, PAL_MODE_OUTPUT_OPENDRAIN);
	palSetPadMode(PHASE_FILTER_GPIO, PHASE_FILTER_PIN3, PAL_MODE_OUTPUT_OPENDRAIN);
	PHASE_FILTER_OFF();

	// Current filter is permanent
	/*
	palSetPadMode(CURRENT_FILTER_GPIO1, CURRENT_FILTER_PIN1, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
	palSetPadMode(CURRENT_FILTER_GPIO2, CURRENT_FILTER_PIN2, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
	palSetPadMode(CURRENT_FILTER_GPIO3, CURRENT_FILTER_PIN3, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
	CURRENT_FILTER_OFF();
	*/
...
void hw_start_i2c(void) {
	...
	if (!i2c_running) {
		...
		palSetPadMode(
			HW_I2C_SDA_PORT, HW_I2C_SDA_PIN,
			PAL_MODE_ALTERNATE(HW_I2C_GPIO_AF) |
			PAL_STM32_OTYPE_OPENDRAIN |
			PAL_STM32_OSPEED_MID1 |
			PAL_STM32_PUDR_PULLUP
...
/* Not present in devkit v3
	// Reset the fault latches in the power stage
...
static void terminal_cmd_doublepulse(int argc, const char** argv) {
	...
	*mcconf = *mc_interface_get_configuration();
	...
	switch (mcconf->motor_type) {
		...
		case MOTOR_TYPE_DC: mcpwm_init(mcconf);
			break;

		case MOTOR_TYPE_FOC: mcpwm_foc_init(mcconf, mcconf);
			break;
